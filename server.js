...
const GROWW_API_KEY = process.env.GROWW_API_KEY || "eyJraWQiOiJaTUtjVXciLCJhbGciOiJFUzI1NiJ9..cjX2QNcSAapKSW4kopVIUdcVNrHlipBjdUpjeQm1RFr20vkc_5I4a_tvth27QZXMJvFej3a7vkjKEG2wK-uDlg";
...
async function authenticateGroww() {
  try {
    ...
    const authRes = await fetch('https://api.groww.in/v1/token/api/access', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + GROWW_API_KEY, 'Content-Type': 'application/json' },
    ...
    if (authRes.ok) {
      ...
      console.error('❌ Failed to get access token:', await authRes.text());
    }
  } catch (e) {
    console.error('❌ Error authenticating with Groww:', e);
  }
}
...
async function fetchGrowwHistory(symbol) {
  try {
    ...
    const response = await fetch(url, { headers: GROWW_HEADERS });
    ...
    if (!response.ok) {
      ...
      const response2 = await fetch(url2, { headers: GROWW_HEADERS });
      if (!response2.ok) {
        console.warn(`  ⚠ History failed for ${symbol}: ${response.status}`);
        return;
      }
...
function processHistoricalData(symbol, data) {
  try {
    // Groww returns candles in various formats depending on API version
    ...
    if (Array.isArray(candles) && candles.length > 0) {
      // Each candle: [timestamp, open, high, low, close, volume] or {open, high, low, close, ...}
      ...
      // Yesterday = second-to-last candle (last one may be today)
...
// ── Groww API: Fetch OHLC for yesterday high/low fallback ──
async function fetchGrowwOHLC(symbol) {
  const url = `https://api.groww.in/v1/live-data/ohlc?exchange=NSE&segment=CASH&trading_symbol=${encodeURIComponent(symbol)}`;
  try {
    const response = await fetch(url, { headers: GROWW_HEADERS });
    if (!response.ok) return null;
    const data = await response.json();
...
async function pollLiveQuotes() {
  try {
    ...
    for (let i = 0; i < FNO_STOCKS.length; i += chunkSize) {
      ...
      // Delay between live quote chunks to avoid rate limits
      await new Promise(r => setTimeout(r, 1000));
    }

    if (growwResults.length === 0) {
      console.warn("⚠ Groww API returned no data this cycle. Check token/secret/rate limits.");
      return;
    }
...
app.get('/api/market-data', (req, res) => {
  if (!latestMarketData) {
    return res.status(503).json({ error: "Data is still initializing. Please wait." });
  }
...
app.listen(PORT, async () => {
  ...
  setInterval(authenticateGroww, 12 * 60 * 60 * 1000);

  await initializeHistory();

  // Poll live quotes every 90 seconds (each cycle takes ~70s for 208 stocks)
